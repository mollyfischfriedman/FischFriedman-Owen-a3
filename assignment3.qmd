...
#MOLLY - This produces three data frames named after their city. However, each data frame has the values only for the last instance of 'j'. Any thoughts?

for(i in url_tables) {
  for(j in east) {
east.page <- read_html(i)
east.tables <- html_table(html_nodes(page, "table"))
east.pop <- as.data.frame(tables[[2]])
east.pop <- east.pop[2:10, -3]
colnames(east.pop) <- paste((j), colnames(east.pop), sep = "_")
output <- data.frame(east.pop)
assign( paste((j)), output)
  } 
}
```
...
text.dataframe <- data.frame(url_tables)
text.dataframe$neighborhood <- str_split(text.dataframe$url_tables, '-', simplify = TRUE)

descriptions$name <- c()
descriptions$name <- text.dataframe$neighborhood
...
tibble <- tibble %>% unnest_tokens(word, text) %>% anti_join(stop_words)
...
```{r}
plot(tibble$word, tibble$neighborhood)
```
